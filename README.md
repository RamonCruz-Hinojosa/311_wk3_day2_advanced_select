...
```
   ...
   select SUM(user_id), state from usersAddress group by state order by state
```
...
SELECT substr(phone1, 1, 3) as areaCode, count(*) as occurrence from usersContact group by areaCode order by occurrence desc
```
...
3.  - first_name: Alaine
    ...
    - county total: 11
      ...
      ```
                ...
                select min(u.first_name), count(ua.county), ua.county from usersAddress as ua
